...
const Article = () => {
  ...
  return (
    <div className='bg-white shadow-md rounded-[24px] border-[1px] border-slate-200 w-full h-auto box-border p-[40px]'>
      ...
      {/* user */}
      <div className='pb-[24px] border-b-[1px] border-b-slate-200 flex justify-between'>
       <div className='flex gap-[30px]'>
       <div className='w-[60px] h-[60px] bg-red-400 rounded-full relative'><Image src={data.user.pic} fill alt='Article Author' /></div>
      ...
      <div className='pt-[24px]'>
        ...
        <div className='text-[48px] font-bold font-family-Roboto'>{data.article.title}</div>
      </div>
...
<div className='text-[24px] font-normal font-family-Roboto'>{data.article.subtitle}</div>
      ...
      <div className='flex flex-col gap-[30px] mt-[40px]'>
        {data.article.content.map((content, index)=>(
          <ContentBlock key={index} sub_unit_name={content.subUnitName} blocks_array={content.blocks} subUnitStyles='text-[24px] font-medium font-family-Roboto' contentBlockStyles='flex flex-col gap-[8px]'/>
        ))}
      </div>
